...
test: tidy
	...
	go test -tags "unit e2e integration" -failfast -vet=off -race -coverprofile=all_coverage.txt -covermode=atomic ./... -ldflags="-X github.com/joshsagredo/oreilly-trial/internal/mail.token=${API_TOKEN}" || (echo an error while testing, exiting!; sh -c 'exit 1';)

.PHONY: test-unit
test-unit: tidy
	$(info starting the unit test for whole module...)
	go test -tags "unit" -failfast -vet=off -race -coverprofile=unit_coverage.txt -covermode=atomic ./... -ldflags="-X github.com/joshsagredo/oreilly-trial/internal/mail.token=${API_TOKEN}" || (echo an error while testing, exiting!; sh -c 'exit 1';)

.PHONY: test-e2e
test-e2e: tidy
	$(info starting the e2e test for whole module...)
	go test -tags "e2e" -failfast -vet=off -race -coverprofile=e2e_coverage.txt -covermode=atomic ./... -ldflags="-X github.com/joshsagredo/oreilly-trial/internal/mail.token=${API_TOKEN}" || (echo an error while testing, exiting!; sh -c 'exit 1';)

.PHONY: test-integration
test-integration: tidy
	$(info starting the integration test for whole module...)
	go test -tags "integration" -failfast -vet=off -race -coverprofile=integration_coverage.txt -covermode=atomic ./... -ldflags="-X github.com/joshsagredo/oreilly-trial/internal/mail.token=${API_TOKEN}" || (echo an error while testing, exiting!; sh -c 'exit 1';)
...
build: tidy
	...
	go build -ldflags="-X github.com/joshsagredo/oreilly-trial/internal/mail.token=${API_TOKEN}" -o bin/main main.go || (echo an error while building binary, exiting!; sh -c 'exit 1';)
